  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {skills.map((skill, index) => (
        <div
          key={index}
          className="group bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-lg p-6 hover:border-orange-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/10"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {skill.skills.map((s, i) => (
              <span
                key={i}
                className="px-3 py-1 bg-slate-700/50 text-slate-200 text-sm rounded-full hover:bg-orange-600/20 hover:text-orange-400 transition-all duration-300 border border-slate-600/50 hover:border-orange-500/30 cursor-default"
              >
                {s}
              </span>
            ))}
